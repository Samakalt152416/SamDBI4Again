...
function getResetGain(layer, useType = null) {
	...
	if (tmp[layer].gainExp.eq(0)) return MetaNumZero
	if (type=="static") {
		if ((!tmp[layer].canBuyMax) || tmp[layer].baseAmount.lt(tmp[layer].requires)) return MetaNumOne
		...
		if (tmp[layer].baseAmount.lt(tmp[layer].requires)) return MetaNumZero
		let gain = tmp[layer].baseAmount.div(tmp[layer].requires).pow(tmp[layer].exponent).times(tmp[layer].gainMult).pow(tmp[layer].gainExp)
		if (gain.gte(tmp[layer].softcap)) gain = gain.pow(tmp[layer].softcapPower).times(tmp[layer].softcap.pow(MetaNumOne.sub(tmp[layer].softcapPower)))
		gain = gain.times(tmp[layer].directMult)
...
function getNextAt(layer, canMax=false, useType = null) {
	...
	if (type=="static") 
	{
		...
		if (next.gte(tmp[layer].softcap)) next = next.div(tmp[layer].softcap.pow(MetaNumOne.sub(tmp[layer].softcapPower))).pow(MetaNumOne.div(tmp[layer].softcapPower))
...
function softcap(value, cap, power = 0.5) {
	if (value.lte(cap)) return value
		...
		return value.pow(power).times(new MetaNum(cap).pow(MetaNumOne.sub(power)))
}
...
function resetBuyables(layer){
	...
	player[layer].spentOnBuyables = MetaNumZero
}
...
function doReset(layer, force=false) {
	...
	if (!force) {
		...
		tmp[layer].baseAmount = MetaNumZero // quick fix
	}
	...
	player.points = (row == 0 ? MetaNumZero : getStartPoints())
